...
export default function Invoices({ state, updateState, setCurrentView }: Props) {
  ...
  const [printLayoutMode, setPrintLayoutMode] = useState<PrintLayout>('thermal');
  ...
  const handleDownloadPDF = async (inv: Invoice) => {
    ...
    try {
      ...
      const format = layout === 'thermal' ? [72, 250] : 'a4';
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[40px] w-full max-w-2xl shadow-2xl flex flex-col border border-white/10 animate-in zoom-in-95">
            <header className="p-8 border-b flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black dark:text-white uppercase tracking-tighter">Inventory Return Protocol</h3>
                  <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-1">Invoice #INV-{returningInvoice.id.padStart(4, '0')}</p>
                </div>
              </div>
           ...
           <div className="bg-white dark:bg-slate-900 rounded-[56px] w-full max-w-2xl shadow-2xl overflow-hidden border border-white/10 flex flex-col animate-in zoom-in-95 duration-300">
              ...
              <div className="flex-1 overflow-y-auto p-12 custom-scrollbar space-y-12">
                ...
                <div className="space-y-6">
                  <label className="block text-[11px] font-black text-indigo-600 uppercase tracking-[0.3em] ml-2">Dispatch Mode / Page Architecture</label>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <div>
                        <p className="font-black text-lg dark:text-white uppercase leading-none">Thermal (72mm)</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">POS Tape Optimized</p>
                      </div>
                    ...
                    >
                      ...
                      <div>
                        <p className="font-black text-lg dark:text-white uppercase leading-none">Formal A4 (210mm)</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">Corporate Ledger Blueprint</p>
                      </div>
                       ...
                       <div className="space-y-2 max-h-[300px] overflow-y-auto no-scrollbar">
                          {state.products.filter(p => !p.isDeleted && p.name.toLowerCase().includes(productSearch.toLowerCase())).slice(0, 8).map(p => (
                             <button key={p.id} onClick={() => setDraftItems([...draftItems, { ...p, quantity: 1, buyPrice: p.price }])} className="w-full p-4 bg-white dark:bg-slate-800 rounded-2xl border text-left hover:border-indigo-500 transition-all flex justify-between items-center group">
                                <div>
                                  <p className="font-black text-xs dark:text-white uppercase truncate w-32">{p.name}</p>
                                  <p className="text-[9px] text-slate-400 font-bold">{state.settings.currency}{p.price.toLocaleString()}</p>
                                </div>
                    ...
                    <footer className="p-8 bg-white dark:bg-slate-900 border-t space-y-6">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        ...
                        {paymentStatusMode === 'partial' && (
                          <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase mb-2 px-1">Amount Paid</p>
                            <input type="number" value={draftPaidAmount} onChange={e => setDraftPaidAmount(Number(e.target.value))} className="w-full bg-slate-50 dark:bg-slate-800 rounded-xl py-2 px-4 font-black text-xs dark:text-white" />
                          </div>
                        )}
                  ...
                  <tr key={inv.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 group transition-all">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        <button onClick={() => handleOpenEditInvoice(inv)} className="p-2 text-slate-400 hover:text-indigo-600 transition-all" title="Edit Invoice Details"><Edit2 size={18}/></button>
                        <button 
                          onClick={() => setPrintingInvoice(inv)} 
                          className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm" 
