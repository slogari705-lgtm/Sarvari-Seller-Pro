  ...
  return (
    <div className={`flex h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-hidden ${isRTL ? 'font-arabic' : ''}`}>
      <aside className={`hidden lg:flex ${sidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-slate-900 border-x transition-all duration-300 flex-col z-40 shadow-sm`}>
        ...
        <div className="p-3 border-t"><button onClick={() => setSidebarOpen(!sidebarOpen)} className="w-full flex items-center justify-center p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">{sidebarOpen ? <X size={16}/> : <Menu size={16}/>}</button></div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-3 lg:p-6 custom-scrollbar"><div className="max-w-7xl mx-auto">{(() => {
          switch (currentView) {
